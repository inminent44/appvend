...
class _CuentaDetalleScreenState extends State<CuentaDetalleScreen> {
  ...
  // ── Cobrar con selección de método de pago ────────────────────────────────
  Future<void> _cobrar() async {
    if (_cuenta == null || _cuenta!.items.isEmpty) return;

    // Mostrar sheet de método de pago
    final pago = await PagoSheet.mostrar(context, _cuenta!.total);
    if (pago == null || !mounted) return;

    setState(() => _cobrando = true);
    try {
      await DBHelperCajero.instance.cobrarCuenta(
        _cuenta!,
        metodoPago:     pago.metodo,
        moneda:         pago.moneda,
        montoMoneda:    pago.montoMoneda,
        tasaCambio:     pago.tasaCambio,
        refTransaccion: pago.refTransaccion,
        plataformaQr:   pago.plataformaQr,
      );
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Row(children: [
          ...
          Expanded(child: Text(
            '${_cuenta!.nombre} — ${_fmt.format(_cuenta!.total)} cobrado (${pago.etiquetaMetodo}) ✓',
          )),
        ]),
  ...
  Widget _itemCard(ItemCuenta item, int index) {
    final colores = [
      const Color(0xFF084B53), const Color(0xFFE53935),
      const Color(0xFFF57C00), const Color(0xFF7B1FA2),
        ...
        children: [
          if (hayItems) ...[
            Row(
              ...
              children: [
                Text('${_cuenta!.cantidadItems} unidad${_cuenta!.cantidadItems != 1 ? 'es' : ''}',
                    style: TextStyle(
                        color: Colors.grey.shade500, fontSize: 13)),
                Text('${_cuenta!.items.length} concepto${_cuenta!.items.length != 1 ? 's' : ''}',
                    style: TextStyle(
                        color: Colors.grey.shade500, fontSize: 13)),
              ],
            ),
...
// ═══════════════════════════════════════════════════════════════════════════
// Sheet selector de productos (sin cambios)
// ═══════════════════════════════════════════════════════════════════════════
