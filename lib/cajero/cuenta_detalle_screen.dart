...
import '../services/catalogo_service.dart'; // ← NUEVO
import 'pos_selector_screen.dart'; // ← NUEVO
...
class _CuentaDetalleScreenState extends State<CuentaDetalleScreen> {
  ...
  Future<void> _cargar() async {
    ...
    CatalogoService.instance.invalidar(); // ← NUEVO
  ...
  // ← NUEVO: reemplaza el showModalBottomSheet anterior
  Future<void> _mostrarSelector() async {
    final itemsMap = <int, double>{
      for (final item in (_cuenta?.items ?? [])) item.productoId: item.cantidad,
    };
    ...
    await Navigator.push(
      ...
      MaterialPageRoute(
        ...
        builder: (_) => PosSelectorScreen(
          nombreCuenta: _cuenta?.nombre ?? '',
          itemsActuales: itemsMap,
      ...
      child: Padding(
        ...
        child: Row(
          children: [
              ...
              child: Row(
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(item.cantidad.toStringAsFixed(0),
                        style: const TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
                            color: Color(0xFF1A1A2E))),
            ...
            SizedBox(
              width: 68,
              child: Text(_fmt.format(item.subtotal),
                ...
                children: [
                  const Text('TOTAL',
                      style: TextStyle(
                          ...
                          fontWeight: FontWeight.bold,
                          letterSpacing: 1)),
                  Text(_fmt.format(_cuenta!.total),
                      style: const TextStyle(
                          fontSize: 28,
                          fontWeight: FontWeight.bold,
                          color: Color(0xFF1A1A2E),
...
// ════════════════════════════════════════════════════════════════════════════
// _TicketCobro — sin cambios respecto al original
// ════════════════════════════════════════════════════════════════════════════
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
              ...
              child:
                  const Icon(Icons.receipt_long, color: primaryDark, size: 28)),
          const SizedBox(height: 12),
          ...
          ...widget.cuenta.items.map((item) => Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Text(widget.fmt.format(item.subtotal),
                        style: const TextStyle(
            ...
            children: [
              const Text('TOTAL',
                  style: TextStyle(
                      ...
                      color: Colors.grey,
                      letterSpacing: 1)),
              Text(widget.fmt.format(widget.cuenta.total),
                  style: const TextStyle(
                      fontSize: 28,
                      fontWeight: FontWeight.bold,
                      color: primaryDark)),
            ...
            children: [
              Expanded(
                child: OutlinedButton(
                  ...
                  onPressed: () =>
                      Navigator.pop(context, {'confirmado': false}),
              ...
              Expanded(
                ...
                child: ElevatedButton(
                  style: ElevatedButton.styleFrom(
                      ...
                      elevation: 3),
                  onPressed: () => Navigator.pop(context,
                      {'confirmado': true, 'metodo': _metodoSeleccionado}),
                  child: const Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
  ...
  Widget _buildMetodoPagoSelector() {
    return SegmentedButton<String>(
      segments: const <ButtonSegment<String>>[
        ...
        ButtonSegment<String>(
            value: 'QR', label: Text('QR'), icon: Icon(Icons.qr_code_2)),
      ],
      selected: {_metodoSeleccionado},
      onSelectionChanged: (Set<String> newSelection) {
        setState(() => _metodoSeleccionado = newSelection.first);
      },
